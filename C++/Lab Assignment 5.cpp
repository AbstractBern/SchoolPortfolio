/*Bernard Flores
Complete
This program uses arrays and many other operations to store business Armadillo Automotive Group data and outputs business calculation amoutns */
//only needs enhanced validation ex: if input is character or whitespace also repeat
#include <iostream>
#include <iomanip>
using namespace std;
...
int main()														//main Function does all the I/O and Operations.
{
	...
	for (num=0; num<EMPLOYEE_NUM; num++,counter++)				//This section asks for input from 4 employees of the AUTO. Biz.
	{															//and loops 4 times each time allocating the info to the Armadillo structure
		...
		while (Armadillo[num].employee_ID < 0 )					//error message
		{
...
//			}
		...
		while (Armadillo[num].payRate <= 0.0)					//error message
		{
			cout<<setw(45)<<"ERROR: ENTER A NUMBER GREATER THAN 0." << endl;
			cout<<setw(45)<<"Pay Rate: ";
			cin>>Armadillo[num].payRate;	
	...
	cout<<setw(50)<<"---------------" << endl;
	for (num1=0; num1<EMPLOYEE_NUM; num1++)						//This part asks for the hours worked for each individual
	{															//it loops 4 times stroring each cell at a time
		cout<<right<<setw(37)<<"Amount of Hours " << Armadillo[num1].employee_Name << " worked: ";
		cin>>employee_TimeCard[num1];
		while (employee_TimeCard[num1] <= 0.0)					//error message
		{
