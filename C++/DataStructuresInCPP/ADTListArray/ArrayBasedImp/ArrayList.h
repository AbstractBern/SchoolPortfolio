...
//**************************************new implementation
template<class ItemType>
bool ArrayList<ItemType>::remove(const ItemType& anEntry)
{
	bool canRemoveItem = (itemCount >= 1);
	if (canRemoveItem)
	{
		int pos;
		...
		for (int entryPos = pos; entryPos < itemCount; entryPos++)
         items[entryPos] = items[entryPos + 1];
        itemCount--;
    }
