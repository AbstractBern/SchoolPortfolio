...
public:
   ...
   /** Gets position of a given entry in a list
   	@pre: 
   	@post: searched entrys' position is returned
   	@param: desired entry to be searched for
   	@return an int <itemCount if entry found, (smallest val returned if multiples are found, else 0. */
   virtual int getPosition(const ItemType& anEntry) const = 0;
    
	/** Removes the entry at a given position from this list.
    @pre  None.
    @post  If 1 <= position <= getLength() and the removal is successful,
       the entry at the given position in the list is removed, and the returned value is true.
    @param data type entry to remove.
    @return  True if removal is successful, or false if not. */
