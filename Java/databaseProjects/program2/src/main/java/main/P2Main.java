...
/**********************************************************************
 cs3743p1Driver.c bya Larry Clark
...
public class P2Main {
    ...
    private static int printAll(String fileName) {
        ...
        for( int pos = 1; pos <= hashFile.getHashHeader().getMaxHash(); pos++) {
            vehicle = new Vehicle();
            MutableInteger loc = new MutableInteger(pos);
            rc = StudentFunctions.readRec(hashFile, loc, vehicle);
            if(rc == ReturnCodes.RC_OK && vehicle.getVehicleId()[1] != '\0') {
                System.out.printf("    %2d", loc.intValue());
                printVehicle(vehicle, hashFile.getHashHeader().getMaxHash());
            }
        }
